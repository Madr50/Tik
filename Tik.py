import webbrowser
webbrowser.open("https://t.me/Vibro_m")
...
def Gmail(email):
    ...
    if "password"in response:
        ...
        ge+=1
        print(f"\r {C}True {K}: {V}{ge} × {HH}False {HH}: {M}{bt} × {Z}Not {a14}: {M}{be} {G}\r")
    else:
        os.system('cls'if os.name=="nt"else"clear")
        be+=1
        print(f"\r {C}True {K}: {V}{ge} × {HH}False {HH}: {M}{bt} × {Z}Not {a14}: {M}{be} {G}\r")
...
def req(email):
    ...
    if ("1023")in re:
         ...
         os.system('cls'if os.name=='nt'else'clear')
         gt+=1
         print(f"\r {F}HlT {X}: {A}{ge} ~ {Z}GOOD {S}: {M}{bt} ~ {C}NOT {HH}: {M}{be} {K}\r")
    else:
        os.system('cls'if os.name=='nt'else'clear')
        bt+=1
        print(f"\r {F}HlT {X}: {A}{ge} ~ {Z}GOOD {S}: {M}{bt} ~ {C}NOT {HH}: {M}{be} {K}\r")
...
'abcdefghijklmnopqrstuvwxyzéèêëàâäôùûüîïç',
                ...
                msToken = zaid.cookies.get_dict()['msToken']                #print(ttwid)
                headers = {
                    'accept': '*/*',
                    'accept-language': 'en-US,en;q=0.9',
